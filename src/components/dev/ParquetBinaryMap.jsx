...
export default function ParquetMap() {
    ...
    const processParquetWithWasm = useCallback(async (parquetUint8Array) => {
        if (!wasmReady) {
            console.error("WASM requested but not ready!");
            throw new Error("Parquet-WASM module is not initialized yet.");
        }
        console.log(`Main thread: Received ${parquetUint8Array.byteLength} bytes for WASM processing.`);
        try {
            ...
            console.log(`Main thread: WASM processing successful. Returning IPC stream (${ipcStream.byteLength} bytes).`);
    ...
    useEffect(() => {
        ...
        return () => {
            console.log("ParquetMap unmounting cleanup.");
            isMountedRef.current = false;
    ...
    useEffect(() => {
        if (fileUrls.length > 0) {
            console.log("File URLs changed, resetting data and loading state.");
            ...
            if (workerPoolRef.current) {
                console.log("Terminating existing worker pool due to file list change.");
                workerPoolRef.current.terminate();
                workerPoolRef.current = null;
            }

        } else {
            console.log("File URLs list is empty.");
            setAllData([]);
            currentDataRef.current = { src: new Float32Array(0), length: 0 };
            setTotalDataLenght(0);
            setProcessedFiles(0);
    ...
    useEffect(() => {
        ...
        async function loadData() {
            ...
            setError(null);
            setErrorMessage(null);
            console.log(`Starting data loading process for ${fileUrls.length} files.`);
            ...
            if (!workerPoolRef.current) {
                try {
                    ...
                    // const workerScriptUrl = new URL('./workers/parquetWorker.js', import.meta.url);
                    // Передаем processParquetWithWasm в конструктор WorkerPool
                    // workerPoolRef.current = new WorkerPool(workerScriptUrl, 10, { type: 'module' }, processParquetWithWasm);
    ...
    const mergeData = useCallback((existingData, newData) => {
        ...
        if (!existingData || existingData.length === 0) {
            console.log(`MergeData: Initial data size: ${newData.length}`);
        ...
        combinedSrc.set(existingData.src);
        combinedSrc.set(newData.src, existingData.length * 6);
        console.log(`MergeData: Merged ${newData.length} records. Total now: ${totalLength}`);
    ...
    const processFilesWithWorkers = useCallback(async (urls) => {
        if (!workerPoolRef.current) {
            console.error("processFilesWithWorkers called but worker pool is not initialized.");
            throw new Error("Worker pool is not available.");
        }
        const BATCH_SIZE = 10; // Размер пакета обработки файлов
        console.log(`Processing ${urls.length} files in batches of ${BATCH_SIZE}...`);

        for (let i = 0; i < urls.length; i += BATCH_SIZE) {
            if (!isMountedRef.current) {
                console.log("Processing stopped: Component unmounted.");
                break; // Прерываем цикл, если компонент размонтирован
            }
            const batchUrls = urls.slice(i, i + BATCH_SIZE);
            console.log(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1}: ${batchUrls.length} files.`);
            ...
            const batchPromises = batchUrls.map(url =>
                workerPoolRef.current.enqueueTask({ url }) // Отправляем URL в воркер
                    .then(result => {
                        ...
                        if (!isMountedRef.current) return null;
                        console.log(`Successfully processed file: ${result?.data?.url || url}`);
                        setProcessedFiles(prev => prev + 1);
                    ...
                    .catch(error => {
                        ...
                        console.error(`Failed to process file ${failedUrl}:`, error.message);
            ...
            if (batchData.length > 0 && isMountedRef.current) {
                console.log(`Merging batch data (${batchData.length} records) into main data.`);
                const newData = mergeData(currentDataRef.current, batchData);
